...
class FastElevator:
    ...
    def __init__(self, elevator_id: int, served_floors: set, config: SimConfig, 
                 env: simpy.Environment, call_store: simpy.Store, statistics: 'FastStats'):
...
class FastDispatcher:
    def __init__(self, config: SimConfig, elevators: List[FastElevator]):
        self.config = config
...
class FastPassengerGenerator:
    def __init__(self, env: simpy.Environment, config: SimConfig, 
                 dispatcher: FastDispatcher, statistics: FastStats, rng: np.random.Generator):
        self.env = env
        self.config = config
...
def run_simulation(strategy: str, seed: int, sim_time: int = 300):
    ...
    dispatcher = FastDispatcher(config=config, elevators=elevators)
    generator = FastPassengerGenerator(env, config, dispatcher, stats, rng)
...
def main():
    print("=" * 80)
    print("电梯分组策略 - SimPy快速仿真验证 (15次/策略)")
    print("=" * 80)
    ...
    for strategy, label in zip(strategies, labels):
        ...
        for i in range(15):
            res = run_simulation(strategy, i * 100 + 42, 300)
    ...
    for strategy, label in zip(strategies, labels):
        ...
        std_wait = np.std(all_results[strategy]['avg_wait'])
        avg_throughput = np.mean(all_results[strategy]['throughput'])
        std_throughput = np.std(all_results[strategy]['throughput'])
        avg_passengers = np.mean(all_results[strategy]['total_passengers'])
