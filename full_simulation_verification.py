...
def run_batch_simulations(strategy: str, n_runs: int = 100, sim_time: int = 300):
    """运行批量仿真"""
...
def plot_comprehensive_results(results_dict, output_prefix):
    ...
    ax1.set_title('平均等待时间对比\n(误差棒表示标准差)', fontsize=13, fontweight='bold')
    ...
    ax3.set_title('等待时间分布\n(箱线图)', fontsize=13, fontweight='bold')
    ...
    ax4.set_title('相对于奇偶分组的改进幅度\n(正数表示改善)', fontsize=13, fontweight='bold')
    ...
    for bar in bars1:
        height = bar.get_height()
        ax4.text(bar.get_x() + bar.get_width()/2., height + 0.5 if height >= 0 else height - 2,
    ...
    ax5.set_title(f'每次仿真平均完成乘客数\n(共100次仿真)', fontsize=13, fontweight='bold')
    ...
    ax6.set_title('综合效率雷达图\n(越大越好)', fontsize=13, fontweight='bold', pad=20)
    ax6.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0), fontsize=10)
    ax6.grid(True)
    
    plt.tight_layout()
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    chart_file = f'{output_prefix}_full_simulation_{timestamp}.png'
...
def main():
    print("=" * 80)
    print("电梯分组策略 - 完整真实仿真验证")
    print("=" * 80)
    print(f"实验时间: {datetime.now()}")
    print(f"仿真时长: 300秒/次")
    print(f"实验次数: 100次/策略")
    ...
    for strategy in strategies:
        results = run_batch_simulations(strategy, n_runs=100, sim_time=300)
        all_results[strategy] = results
    
    print("\n" + "=" * 80)
    print("生成结果图表...")
    chart_file = plot_comprehensive_results(all_results, 'elevator_strategy')
    print(f"图表已保存: {chart_file}")
    
    print("\n" + "=" * 80)
    print("仿真结果统计 (100次仿真平均)")
    print("=" * 80)
