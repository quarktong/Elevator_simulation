...
class ElevatorFast:
    ...
    def __init__(self, elevator_id: int, served_floors: set, config: SimConfig, 
                 env: simpy.Environment, call_store: simpy.Store, statistics: 'StatisticsFast'):
    ...
    def _handle_door_open(self, floor):
        ...
        yield self.env.timeout(0.5)  # 快速开关门
    ...
    def _move_to_floor(self, target):
        while round(self.position) != target:
            if self.direction == Direction.UP:
                self.position += 1
                yield self.env.timeout(0.1)  # 快速移动
            elif self.direction == Direction.DOWN:
                self.position -= 1
...
class DispatcherFast:
    def __init__(self, config: SimConfig, elevators: List[ElevatorFast]):
        self.config = config
...
class PassengerGeneratorFast:
    def __init__(self, env: simpy.Environment, config: SimConfig, 
                 dispatcher: DispatcherFast, statistics: StatisticsFast, rng: np.random.Generator):
        self.env = env
        self.config = config
...
def run_fast_simulation(strategy: str, seed: int, sim_time: int = 300):
    ...
    dispatcher = DispatcherFast(config=config, elevators=elevators)
    generator = PassengerGeneratorFast(env, config, dispatcher, stats, rng)
...
def run_batch_simulations(strategy: str, n_runs: int = 50, sim_time: int = 300):
    print(f"运行 {strategy} 策略 ({n_runs} 次)...")
    ...
    for i in range(n_runs):
        res = run_fast_simulation(strategy, i * 100 + 42, sim_time)
        ...
        if (i + 1) % 10 == 0:
            print(f"  {i+1}/{n_runs} 完成")
    
    return results

def plot_results(results_dict, output_prefix):
    ...
    ax3.set_title('等待时间分布', fontsize=14, fontweight='bold')
    ...
    wait_improve = [(base_wait - np.mean(results_dict[s]['avg_wait']))/base_wait * 100 for s in strategies]
    throughput_improve = [(np.mean(results_dict[s]['throughput']) - base_throughput)/base_throughput * 100 for s in strategies]
    ...
    ax4.set_title('相对于奇偶分组的改进', fontsize=14, fontweight='bold')
    ...
    ax5.set_title('每次仿真完成乘客数', fontsize=14, fontweight='bold')
    ax5.grid(axis='y', alpha=0.3)
    
    # 6. 雷达图
    ax6 = plt.subplot(2, 3, 6, polar=True)
    categories = ['等待时间', '吞吐量', '乘客数', '稳定性']
    wait_scores = [100 - (np.mean(results_dict[s]['avg_wait']) / max([np.mean(results_dict[x]['avg_wait']) for x in strategies])) * 100 for s in strategies]
    throughput_scores = [(np.mean(results_dict[s]['throughput']) / max([np.mean(results_dict[x]['throughput']) for x in strategies])) * 100 for s in strategies]
    passenger_scores = [(np.mean(results_dict[s]['total_passengers']) / max([np.mean(results_dict[x]['total_passengers']) for x in strategies])) * 100 for s in strategies]
    stability_scores = [100 - np.std(results_dict[s]['avg_wait']) for s in strategies]
    
    values = np.array([[wait_scores[i], throughput_scores[i], passenger_scores[i], stability_scores[i]] for i in range(3)])
    ...
    ax6.set_title('综合效率对比', fontsize=14, fontweight='bold', pad=20)
    ax6.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0), fontsize=10)
    ax6.grid(True)
    
    plt.tight_layout()
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    chart_file = f'{output_prefix}_verified_{timestamp}.png'
...
def main():
    print("=" * 80)
    print("电梯分组策略 - 真实SimPy仿真验证")
    print("=" * 80)
    print(f"实验时间: {datetime.now()}")
    print(f"仿真时长: 300秒/次")
    print(f"实验次数: 50次/策略")
    print(f"使用SimPy事件驱动仿真引擎")
    print("=" * 80)
    ...
    for strategy in strategies:
        results = run_batch_simulations(strategy, n_runs=50, sim_time=300)
        all_results[strategy] = results
    
    print("\n生成图表...")
    chart_file = plot_results(all_results, 'elevator_verified')
